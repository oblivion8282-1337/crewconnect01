    ...
    >
        ...
        {!phaseHasDates && (
          <div className="border-b bg-blue-50">
            ...
            >
              ...
              <ChevronDown className={`w-4 h-4 text-blue-600 transition-transform ${showDatePicker ? 'rotate-180' : ''}`} />
            </button>
            {showDatePicker && (
        ...
        <div className="p-4 border-b bg-gray-50">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
            ...
            {/* Berufsfeld */}
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Berufsfeld</label>
              <select
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {!hasValidDateRange ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 font-medium">Bitte wähle zuerst einen Zeitraum</p>
              <p className="text-sm text-gray-400">Wähle oben Start- und Enddatum für die Buchung</p>
            </div>
          ) : filteredFreelancers.length === 0 ? (
            <div className="text-center py-12">
              <Users className="w-12 h-12 text-gray-300 mx-auto mb-3" />
              <p className="text-gray-500">Keine Freelancer gefunden</p>
              <p className="text-sm text-gray-400">Passe die Filterkriterien an</p>
            </div>
          ) : (
  ...
  return (
    <div className={`bg-white rounded-xl border-2 transition-colors ${
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              <div>
                ...
                <p className="text-gray-600 text-sm">{(freelancer.professions || []).join(', ')}</p>
              </div>
              ...
              <div className="text-right flex-shrink-0">
                ...
                {visibility.dayRate !== false && freelancer.dayRate && (
                  <p className="text-sm font-bold text-gray-700">{freelancer.dayRate}€/Tag</p>
                )}
              </div>
            </div>
      ...
      <div className="px-4 pb-3">
        <div className="p-3 bg-gray-50 rounded-lg">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-3 pb-3 border-b border-gray-200">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-3 py-1.5 text-xs font-medium bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors flex items-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed"
              >
          ...
          <div className="flex flex-wrap gap-4 mt-3 text-xs text-gray-500">
            ...
            <span className="flex items-center gap-1">
              <div className="w-3 h-3 rounded bg-gray-200" /> Wochenende
            </span>
          </div>
        </div>
      </div>
      ...
      <button
        ...
        className="w-full py-2 border-t text-sm text-gray-600 hover:bg-gray-50 flex items-center justify-center gap-1"
      >
      ...
      {expanded && (
        <div className="p-4 border-t bg-gray-50 space-y-3">
          {visibility.bio !== false && freelancer.bio && (
            <div>
              <h4 className="text-xs font-medium text-gray-500 mb-1">Über mich</h4>
              <p className="text-sm text-gray-700">{freelancer.bio}</p>
            </div>
          )}
          ...
          {visibility.equipment !== false && freelancer.equipment?.length > 0 && (
            <div>
              <h4 className="text-xs font-medium text-gray-500 mb-1">Equipment</h4>
              <div className="flex flex-wrap gap-1">
                {freelancer.equipment.map(item => (
                  <span key={item} className="px-2 py-0.5 bg-purple-100 text-purple-700 rounded text-xs">
                    {item}
                  </span>
                ))}
              </div>
          ...
          {freelancer.languages?.length > 0 && (
            <div>
              <h4 className="text-xs font-medium text-gray-500 mb-1">Sprachen</h4>
              <div className="flex flex-wrap gap-1">
                {freelancer.languages.map(lang => (
                  <span key={lang} className="px-2 py-0.5 bg-green-100 text-green-700 rounded text-xs">
                    {lang}
                  </span>
                ))}
              </div>
          ...
          <div className="flex flex-wrap gap-3 pt-2">
            {visibility.email !== false && freelancer.email && (
              <a href={`mailto:${freelancer.email}`} className="text-xs text-blue-600 hover:underline flex items-center gap-1">
                <Mail className="w-3.5 h-3.5" />
                {freelancer.email}
              </a>
            )}
            {visibility.phone !== false && freelancer.phone && (
              <a href={`tel:${freelancer.phone}`} className="text-xs text-blue-600 hover:underline flex items-center gap-1">
                <Phone className="w-3.5 h-3.5" />
                {freelancer.phone}
              </a>
            )}
            {visibility.website !== false && freelancer.website && (
              <a href={freelancer.website} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-600 hover:underline flex items-center gap-1">
      ...
      {/* Buchungsoptionen */}
      {showBookingOptions && (
        <div className="p-4 border-t bg-blue-50 space-y-3">
          {/* Erste Zeile: Buchungstyp und Gage */}
          <div className="flex flex-wrap items-center gap-4">
            {/* Buchungstyp: Option/Fix */}
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Buchungstyp</label>
            ...
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Gagentyp</label>
              <div className="flex gap-1">
                <button
          ...
          <div className="flex items-center justify-between pt-2 border-t border-blue-200">
            <div className="text-sm">
              ...
              {rateType === 'daily' && (
                <span className="text-gray-500 ml-2">× {customDayRate}€/Tag</span>
              )}
              <span className="text-gray-500 mx-2">=</span>
              <span className="font-bold text-blue-700 text-lg">{totalCost.toLocaleString('de-DE')}€</span>
              {rateType === 'flat' && (
                <span className="text-gray-500 ml-1 text-xs">(pauschal)</span>
              )}
            </div>
